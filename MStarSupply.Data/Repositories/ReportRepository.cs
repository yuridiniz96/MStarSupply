...
namespace MStarSupply.Data.Repositories
{
    public class ReportRepository
    {
        public byte[] CreateExcel(List<EntradaSaida> entradasSaidas)
        {
            ...
            {
                var planilha = excelPackage.Workbook.Worksheets.Add("Relatório");
        ...
        public byte[] CreatePdf(List<EntradaSaida> entradasSaidas)
        {
            ...
            using (var document = new Document(pdf))
            {
                ...
                ("https://media-exp1.licdn.com/dms/image/C4E0BAQG-hdPIX1pMeA/company-logo_200_200/0/1613670796599?e=1678924800&v=beta&t=vo_GwW1HuLmTxoI5LHLxeetCeQj5TM7rx3KweuVvzIs");
                document.Add(new iText.Layout.Element.Image(logo));
                document.Add(new Paragraph("Relatório de Entradas e Saídas de Mercadoria").AddStyle(new Style().SetFontSize(20)));
                ...
                table.AddHeaderCell(new Paragraph("ID Entrada").AddStyle(new Style().SetFontSize(12)));
                table.AddHeaderCell(new Paragraph("ID Mercadoria").AddStyle(new Style().SetFontSize(12)));
                table.AddHeaderCell(new Paragraph("Mercadoria").AddStyle(new Style().SetFontSize(12)));
                table.AddHeaderCell(new Paragraph("Data de Entrada").AddStyle(new Style().SetFontSize(12)));
                table.AddHeaderCell(new Paragraph("Data de Saída").AddStyle(new Style().SetFontSize(12)));
                
                foreach (var item in entradasSaidas)
                {
                    table.AddCell(new Paragraph(item.IdEntradaSaida.ToString()).AddStyle(new Style().SetFontSize(9)));
                    table.AddCell(new Paragraph(item.IdMercadoria.ToString()).AddStyle(new Style().SetFontSize(9)));
                    table.AddCell(new Paragraph(item.NomeMercadoria.ToString()).AddStyle(new Style().SetFontSize(9)));
                    table.AddCell(new Paragraph(item.DataEntrada.ToString("dd/MM/yyyy")).AddStyle(new Style().SetFontSize(9)));
                    if(item.IsSaida is true)
                    {
                        table.AddCell(new Paragraph(item.DataSaida.ToString("dd/MM/yyyy")).AddStyle(new Style().SetFontSize(9)));
                    }
                    else
                    {
                        table.AddCell(new Paragraph("").AddStyle(new Style().SetFontSize(9)));
                    }
